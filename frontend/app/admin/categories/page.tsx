...
export default function AdminCategoriesPage() {
  ...
  async function deleteCategory(c: Category) {
    ...
    const warn =
      count > 0
        ? `"${c.name}" is currently used by ${count} app${count === 1 ? "" : "s"}. ` +
          "Deleting it will un-tag those apps. Continue?"
        : `Delete "${c.name}"?`;
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-ink-soft">
            The taxonomy used to group apps in the catalogue and in F-Droid
            clients. Renaming a category propagates everywhere; deleting one
            simply un-tags the apps that referenced it.
          </p>
        </div>
      ...
      <section className="surface p-6">
        <h2 className="mb-4 text-lg font-bold tracking-tight text-ink">Add a category</h2>
        <form onSubmit={createCategory} className="grid gap-3 md:grid-cols-[1fr_2fr_auto]">
          <Field label="Name" htmlFor="cname">
            <Input id="cname" required maxLength={64} value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Productivity" />
          </Field>
          <Field label="Description (optional)" htmlFor="cdesc">
            <Input id="cdesc" maxLength={255} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="One-line hint for users browsing this group." />
          </Field>
